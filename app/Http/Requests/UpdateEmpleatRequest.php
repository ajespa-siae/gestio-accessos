...
class UpdateEmpleatRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'nif' => 'required|string|max:20|unique:empleats,nif,' . $this->empleat->id . ',id,estat,actiu',
    ...
    public function messages(): array
    {
        return [
            ...
            'nif.unique' => 'Ja existeix un altre empleat actiu amb aquest NIF',
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            if ($this->nifJaExisteixEnAltreEmpleatActiu()) {
                $validator->errors()->add('nif', 'Ja existeix un altre empleat actiu amb aquest NIF');
            }
        });
    }

    /**
     * Comprovar si ja existeix un altre empleat actiu amb aquest NIF
     */
    protected function nifJaExisteixEnAltreEmpleatActiu(): bool
    {
        return Empleat::where('nif', $this->nif)
            ->where('estat', 'actiu')
            ->where('id', '!=', $this->empleat->id)
            ->exists();
    }
}
