...
<div class="container">
    <div class="row mb-4">
        ...
        <div class="col-md-4 text-end">
            <a href="{{ route('empleats.show', $empleat) }}" class="btn btn-secondary">
                <i class="bi bi-arrow-left"></i> Tornar al detall
            </a>
        </div>
    </div>
            ...
            <form action="{{ route('empleats.update', $empleat) }}" method="POST">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control @error('nom_complet') is-invalid @enderror" 
                            id="nom_complet" name="nom_complet" value="{{ old('nom_complet', $empleat->nom_complet) }}" required>
                        @error('nom_complet')
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control @error('nif') is-invalid @enderror" 
                            id="nif" name="nif" value="{{ old('nif', $empleat->nif) }}" required>
                        @error('nif')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="email" class="form-control @error('correu_personal') is-invalid @enderror" 
                            id="correu_personal" name="correu_personal" value="{{ old('correu_personal', $empleat->correu_personal) }}" required>
                        ...
                        <select class="form-select @error('departament_id') is-invalid @enderror" 
                            ...
                            @foreach($departaments as $departament)
                                <option value="{{ $departament->id }}" 
                                    {{ old('departament_id', $empleat->departament_id) == $departament->id ? 'selected' : '' }}>
                                    {{ $departament->nom }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('carrec') is-invalid @enderror" 
                        id="carrec" name="carrec" value="{{ old('carrec', $empleat->carrec) }}" required>
                    @error('carrec')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('observacions') is-invalid @enderror" 
                        id="observacions" name="observacions" rows="3">{{ old('observacions', $empleat->observacions) }}</textarea>
                    @error('observacions')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-save"></i> Actualitzar Empleat
                    </button>
                </div>
            </form>
        </div>
